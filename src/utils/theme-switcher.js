...
// - Single-brand sites: leave as ['card'] (toggle becomes no-op).
// - Multi-brand sites: add brand keys, e.g. ['card', 'water', 'matter'].
// ============================================================================

const VALID_THEMES = ['card', 'sheet', 'aperture'];

export class ThemeSwitcher {
  constructor() {
    this.currentTheme = this.getStoredTheme() || 'card';
  ...
  getStoredTheme() {
    ...
    return VALID_THEMES.includes(stored) ? stored : 'card';
  }
  ...
  applyTheme(theme, initialLoad = false) {
    ...
    const next = VALID_THEMES.includes(theme) ? theme : 'card';
  ...
  getCurrentTheme() {
    ...
    const html = document.documentElement;
    return VALID_THEMES.find((t) => html.classList.contains(`theme-${t}`)) || 'card';
  }
